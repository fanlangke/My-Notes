...
10. mv移动文件，mv index.html src 当然这样写，必须移动的文件和目标文件
    夹在同一目录下。
11. reset重新初始化终端/清屏。
12. clear清屏。
13. history查看命令历史。
...
git commit -m "消息内容" [filename] //提交暂存区的内容到本地仓库
...
git+ssh://git@192.168.53.168/VT.git # 增加远程定义（用于push/pull/fetch）
git branch                                                # 显示本地分支
git branch 分支名
#创建分支
git branch -v
